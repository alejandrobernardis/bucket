...
b = 'I028005008006011105005122002116002068117024012119004003024006111' \
...
from casino8.security.iron_man import SECRET_HASH, IronMan
iron_man = 'I%s' % IronMan.doit(b, SECRET_HASH)
...
def session_start_device_w_fb():
    for x in (1161647266, 1336069122,):
        req = get_request('do/device/config', fbuid=str(x))
        try:
            data = urlopen(req)
            if not data:
