package yr.mx.snax.pcq {
	import yr.mx.snax.pcq.components.WordAlert;
	...
	public class VideoApplication extends KCComponent implements IKCComponent {
		...
		private var _video_ads_view:Boolean;
		private var _video_ads_id:String;
		private var _video_words:VideoWords;
		private var _video_word_alert:WordAlert;
		...
		override protected function $config(e:Event):void {
			...
			_video_words = new VideoWords();
			_video_words.addEventListener(VideoWords.BLOCK, VideoWordBlock);
			_video_words.addEventListener(VideoWords.UNBLOCK, VideoWordBlock);
			_video_words.addEventListener(VideoWords.START, VideoWordView);
			
			SimpleLog.dump(flash_vars);
		}

		private function VideoWordBlock(e:Event = null):void {
			_video_holder.visible = e.type == VideoWords.BLOCK ? false : true;
		}

		private function VideoWordView(e:Event = null):void {
			var target:VideoWords = e.currentTarget as VideoWords;
			_video_word_alert = new WordAlert(target.word);
			_video_word_alert.visible = false;
			_video_word_alert.position(stage.stageWidth/2, stage.stageHeight/2);
			_video_word_alert.addEventListener(MouseEvent.CLICK, CloseVideoWordView);
			addChild(_video_word_alert);
			try{
				GAHit("view-word", _video_words.word);
			}catch(error:Error){ SimpleLog.print("GA", error.message); }
		}

		private function CloseVideoWordView(e:MouseEvent = null):void {
			if(_video_word_alert && contains(_video_word_alert)){
				_video_holder.visible = true;
				_video_word_alert.removeEventListener(MouseEvent.CLICK, CloseVideoWordView);
				removeChild(_video_word_alert);
				_video_word_alert = null;
			}
		}
		...
		private function RandomElement(coef:Number=4, min:Number=0, max:Number=3):Number {
			...
			return NumberUtil.limits(value, min, max);
		}
		...
		private function CompleteDataHandler(e:Event):void {
			...
			if(_external_data.hasOwnProperty("ADS")) {
				_video_ads_id = _external_data.ADS.vid;
		...
		private function GAHit(level:String, value:String, context:String = "snax-application"):void {
			SimpleLog.tracking(context, level, value);
			if(!TypeUtil.isBoolean(flash_vars.analytics)) return;
		...
		private function PlayerReadyHandler(e:Event=null):void {
			...
			_player.setSize(w, h);
			_player.cueVideoById(_video_ads_id || _current_data.vid);
			...
			try{
				if(!_video_ads_view && _video_ads_id){
					GAHit('view', "ads");
				}else{ GAHit('click', _tracker_list.join("_")); }
			}catch(error:Error){ SimpleLog.print("GA", error.message); }
			...
			switch(action){
				case STATE_ENDED:
					...
					if(_video_ads_view && !_video_ads_id){
						...
						_video_ads_id = null;
						_video_ads_view = true;
						_player.loadVideoById(_current_data.vid);
						try{
				...
				break;
				case STATE_PLAYING:
					if(_video_word_alert){
						_video_word_alert.visible = true;
					}
				break;
			}
		}
