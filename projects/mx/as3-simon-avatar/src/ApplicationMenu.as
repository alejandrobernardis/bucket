	 ...
	 */
 	public class ApplicationMenu extends KCComponent {
		...
		public function set user_name(value:String):void{
			var status:Boolean = (!value || StringUtil.isEmpty(value));
			TextField(getChildByName("txTitle1")).text = status ? "" : "Hola";
			TextField(getChildByName("txTitle2")).text = status ? "" : StringUtil.capitalize(value);
		}
		...
		public function set enabled_menu(value:Boolean):void{
			...
			var list:Array = new Array("mcLogo", "btInstructions", "btExit");
			while(list.length){
