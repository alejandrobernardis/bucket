...
from com.feedback.core.utils import secret_key, activation_key, safe_str_cmp
from com.feedback.models.base import BaseDocument, PasswordField, \
    PermissionField
...
class User(BaseDocument):
    ...
    dispatch = IntField(1, 4, default=4, required=True)
    ...
    executives = ListField(StringField(max_length=32), default=list)
    ...
    remote_ip = StringField(max_length=64, default='127.0.0.1')
    ...
    def set_last_login(self, value=None, data=None):
        return self._datetime_update('last_login', value, data)
    ...
    @staticmethod
    def get_by_sid(value, enabled=True, available=True):
        return User.get_by__first(Q(sid=value), enabled, available)
...
class PreRegister(BaseDocument):
    company = StringField(max_length=128, required=True)
    position = StringField(max_length=128, default='')
    provider = IntField(1, 3, default=1, required=True)
    first_name = StringField(max_length=128)
    last_name = StringField(max_length=128)
    email = EmailField(max_length=255, required=True, unique=True)
