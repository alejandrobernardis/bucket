...
import emails
import settings
from addicted.verify.core.regex import regex_activation_key, \
    regex_activation_key_64, regex_object_id
from addicted.verify.core.utils import safe_str_cmp, token_b64, token, \
    user_token
from addicted.verify.forms.auth import SignInForm, RequestForm, RecoveryForm, \
...
from addicted.verify.security.session import verify_session_status
...
class AuthBaseHandler(BaseHandler):
    ...
    def render(self, **kwargs):
        ...
        super(AuthBaseHandler, self).render(self._template, **kwargs)


class SignInHandler(AuthBaseHandler):
    _form = SignInForm
...
class RequestHandler(AuthBaseHandler):
    _form = RequestForm
        ...
        if form.validate():
            try:
                ...
                try:
                    ...
                    register.first_name = form.first_name.data
                    register.last_name = form.last_name.data
                ...
                try:
                    send_mail(
                        u'Addicted / Socios /  Verificación de datos',
                        emails.CONFIRMATION % {
...
class RequestVerifyHandler(AuthBaseHandler):
    _form = RecoveryForm
    ...
    def get(self, *args, **kwargs):
        ...
        try:
            send_mail(
                u'Addicted / Socios / Verificación de usuario',
                emails.ACTIVATION % {
                    'site_domain': self.settings.get('site_domain'),
...
class RequestVerifyAllowDenyHandler(AuthBaseHandler):
    _form = RequestForm
...
class RecoveryHandler(AuthBaseHandler):
    _form = RecoveryForm
...
class RecoveryVerifyHandler(AuthBaseHandler):
    ...
    def activation_key_verify(self, key):
        ...
        # TODO: Agregar a la base el secure_key y comprobarlo posteriormente.
...
class RecoveryPasswordHandler(AuthBaseHandler):
